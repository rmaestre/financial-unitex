...
class UniteManager():
    ...
    def grammar(self, tokens, pos, lang):
        ...
        self.manager.Locate('-t%s/%s_converted.snt'%(self.conf['tmp_dir'], process_id),'%s/Graphs/%s'%(self.conf['unitex_dir'], self.grammar),'-a%s/Alphabets/%s/Alphabet.txt'%(self.conf['unitex_dir'], lang),'-L','-R','-n100','-z','-Y', *delas)
